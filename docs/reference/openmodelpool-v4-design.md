...
>
> **核心叙事**：OpenModelPool 是一个**临时 Token 银行** + **极客共享网络**。用户把本月用不完、过期会浪费的模型额度存入网络；当别人实际使用这些额度后，系统记录公益额度·贡献记账（非货币）；未来用户需要自己没有的模型时，可以用公益额度·贡献记账（非货币）取回等价模型资源。如果用户永远不取回，则这些贡献自然成为对社区的极客共享。
>
> **产品本质**：OpenModelPool **默认是个人模型代理**。只有当用户配置了 Provider Token、开启额度管理，并且检测到本月存在闲置额度时，系统才在管理员界面提示用户可以**主动加入共享网络**。加入共享网络后才生成助记词和 Node ID，公益额度·贡献记账（非货币）绑定 Node ID，Provider Token 始终只保存在本地。
>
...
OpenModelPool 是一个**创新的去中心化 AI 资源公益共享网络**。它同时也是一个**临时 Token 银行**：用户把本月用不完、过期会浪费的模型额度"存入"网络，当别人实际使用后系统记录公益额度·贡献记账（非货币），未来用户可以用积分"取回"等价模型资源。
...
临时 Token 银行叙事：
  ...
  → 存入 OpenModelPool 网络 → 别人使用后你获得公益额度·贡献记账（非货币）
  → 未来你需要 Claude-3 → 用公益额度·贡献记账（非货币）取回等价资源
...
| 模式 | 默认状态 | 是否联网 | 是否生成 Node ID | 是否生成助记词 | 是否公益额度·贡献记账（非货币） |
|------|---------|---------|-----------------|---------------|-------------|
...
只有**同时满足**以下三个硬条件时，管理员界面才允许用户开启共享网络：

```
入网硬条件（全部满足方可开启）：
  ✅ 已配置至少一个 Provider Token
  ✅ 额度管理已开启
  ✅ 至少有一个 Key 的访问控制设为「共享」
```

此外，`remaining_quota > 0` 为**软提示**条件，仅用于展示"您有闲置额度"的温和提醒，不阻塞入网。

**提示文案**：
```
发现你本月还有闲置模型额度。
你可以选择将部分闲置额度共享到 OpenModelPool 网络。
别人实际使用后，你会获得公益额度·贡献记账（非货币）。
未来你需要自己没有的模型时，可以用公益额度·贡献记账（非货币）调用其他节点模型。
...
| **加入共享**（可选） | 加入共享网络获取更多模型 | 公益额度·贡献记账（非货币）换取等价资源 |
...
| **公益额度·贡献记账（非货币） (non-currency contribution ledger)** | 节点为网络提供的有效服务量的记账单位，不可交易/提现，用于未来取回等价资源 |
| **贡献权重 (Weight)** | 基于公益额度·贡献记账（非货币）计算的调度优先级，类比 BT 的分享率 |
...
| **助记词 (Mnemonic)** | BIP39 助记词，用于生成/恢复 Node ID 和公益额度·贡献记账（非货币），不包含 Provider Token |
...
- **额度约束**：额度由公共池动态分配，但**必须受以下多重上限约束**：
  ...
  - 单模型上限（每种模型分配固定额度）
- **不参与贡献权益**：使用公共 Key 不产生公益额度·贡献记账（非货币），不享受路由优先
...
v4.0 移除旧版复杂 Credit 系统和金融化积分系统，但**保留轻量 non-currency contribution ledger（公益额度·贡献记账（非货币））**作为资源互助的记账单位。采用**节点主人自主决定的精细管控分配模型**。核心围绕两级开关（`network_enabled` + `share_to_pool`），在添加每个 Provider Key 时精细化设定共享额度、模型分配比例、有效期和限流策略。
...
优先级 3：Ticket 记账时的处理
  ...
  └── 无 request-id 的调用不生成 Ticket，不计入公益额度·贡献记账（非货币）
```
...
| 全局公益额度·贡献记账（非货币）池 | 无全局状态，各节点独立设定 | 分布式一致性成本过高 |
...
**公益额度·贡献记账（非货币）（non-currency contribution ledger）定义**：
...
移除的是旧版复杂 Credit 系统（动态阈值解锁、信誉评级、金融化积分）；保留的是轻量 non-currency contribution ledger 作为资源互助的记账单位。
...
**request-id 缺失的处理策略**：
- 上游 API 未返回 `request-id` 的调用，视为**不可验证调用**，**不计入公益额度·贡献记账（非货币）体系**
...
| 组件 | 用途 | 成本 |
|------|------|------|
| **IPFS（预留，未实现）** | 设计为存储关键贡献记录/能力声明；**当前未接入真实 IPFS 节点**，贡献记录仅本地内容哈希（`sha256:`）存储 | 预留 |
...
重新安装 → 默认进入个人版 → 点击"恢复共享节点身份"
  → 输入助记词 → 恢复 Node ID → 同步公益额度·贡献记账（非货币）
...
| 轻量公益额度·贡献记账（非货币） | non-currency contribution ledger（非金融化） |
...
| **公益额度·贡献记账（非货币）保留** | 移除旧版复杂 Credit 系统，保留轻量 non-currency contribution ledger 作为记账单位 |
