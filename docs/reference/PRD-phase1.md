...
**Phase 1 目标（最小闭环）**：在 Phase 0 个人版 MVP（34+ 提供商、本地额度管理、密钥体系 v4）之上，引入**双模式架构 + 助记词身份 + 单跳中继 + 传输路径加密 + 公益额度·贡献记账（非货币）账本 + Public Global Key 四重限额**，打通"请求节点 → Seed 中继 → 资源节点"的可运行闭环，**绝不破坏个人版**。

**用户价值**：
- 个人版用户：本地代理体验完全不受影响，永不被强制升级。
- 共享贡献者：把闲置额度安全共享出去，获得绑定身份的、不可提现/交易的公益额度·贡献记账（非货币）与网络访问权。
...
- **US-5（共享贡献者 / M5、M7、M6）**：作为资源节点，我希望我的节点通过 Seed 中继把请求转发给其他节点且传输路径加密（中继不可见），并按 request-id 累计公益额度·贡献记账（非货币），以便我安全、可计量地贡献。
...
- **US-10（网络参与者 / M6）**：作为网络参与者，我希望我的公益额度·贡献记账（非货币）（不可提现/交易、绑定 Node ID）被准确记录，以便我获得应有的网络权利。
...
#### REQ-11（M6）公益额度·贡献记账（非货币）账本（non-currency contribution ledger）
- **描述**：记录本节点作为资源被调用（`share_to_pool` 开启且请求带 `request-id`）时获得的公益额度·贡献记账（非货币）；消费（调用他人 Provider）时扣减。积分**不可提现/交易、绑定 Node ID、每日上限**。Phase 1 采用**本节点账本 + Seed 聚合视图**（非全量 P2P 账本同步，BFT 留待后续）。
- **所属模块**：账本与积分
...
graph TD
    ...
    M3 --> M6[公益额度·贡献记账（非货币）账本<br/>M6·P1]
...
│  • 公益额度·贡献记账（非货币）不可提现/交易，绑定 Node ID          │
...
### Q2. 中继计费的积分换算规则？
- **现状**：FEDERATION 仅说"提供 Provider 被调用时获得公益额度·贡献记账（非货币）"，未给公式。
- **建议默认值**：
  - 赚取：`公益额度·贡献记账（非货币） += round(成功转发消耗的 Token 数 / 1000)`（即每 1K token ≈ 1 分）；
...
### Q3. 助记词丢失的找回策略？
- **现状**：FEDERATION 明确"丢失助记词 = 丢失 Node ID 身份和公益额度·贡献记账（非货币）"。
...
- **不发币、不提现、不交易**：公益额度·贡献记账（非货币）永久不可兑换法币/金融资产（已定原则）。
...
| `admin.html` `netCredits` 槽位 (L705) | 展示公益额度·贡献记账（非货币）账本（REQ-11） |
