...
当请求的模型在当前节点不存在时，系统按节点列表顺序遍历，找到第一个拥有该模型的可用节点即发起请求，消耗目标节点的共享额度池。

- **Guest Key / Admin Key（Proxy Key）**：优先在本机查找，找不到时路由到其他节点的共享池。
- **Public Key**：始终从共享池调度，本机无该模型时同样路由到其他节点。
