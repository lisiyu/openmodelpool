...
|------|:---------:|:---------:|:---------:|
| 访问本机 Private Provider | ❌ | ✅ | ✅ |
| 访问本机 Shared Provider | ❌ | ✅ | ✅ |
| 访问本机 Public Provider | ❌ | ✅ | ✅ |
| 访问其他节点资源 | ❌ | ❌ | ❌ |
| 查看管理面板 | ❌ | ❌ | ✅ |
| 编辑 Provider | ❌ | ❌ | ✅ |
| 发放 Guest Key | ❌ | ❌ | ✅ |
...
| 访问本机 Shared Provider | ✅ | ✅ | ✅ | ✅ |
| 访问本机 Public Provider | ✅ | ✅ | ✅ | ✅ |
| 访问其他节点 Shared Provider | ✅ | ✅ | ✅ | ✅ |
| 访问其他节点 Public Provider | ✅ | ✅ | ✅ | ✅ |
| 访问其他节点 Private Provider | ❌ | ❌ | ❌ | ✅ |
| 查看管理面板 | ❌ | ❌ | ✅ | ✅ |
| 编辑 Provider | ❌ | ❌ | ✅ | ✅ |
| 发放 Guest Key | ❌ | ❌ | ❌ | ✅ |
| 管理共享网络 | ❌ | ❌ | ❌ | ✅ |
...
| **ShareToPool** | `true` | 是否将此 Provider 共享到网络池（供 Public Key 和其他节点的 Guest Key 访问） |

- **Private Provider**：`AllowGuest=false` + `ShareToPool=false` → 仅 Admin Key 可访问
- **Shared Provider**：`AllowGuest=true` + `ShareToPool=true`（默认） → Guest Key + Public Key 可访问
- **Public Provider**：`AllowGuest=false` + `ShareToPool=true` → 仅 Public Key 可访问（Guest Key 不可）
