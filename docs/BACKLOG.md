...
- [x] P2-1 社区共治设计：节点准入 / 模型白名单多签共治（治理哲学 2026-08-09 已定：默认善意、只防滥发提案、贡献者共治、无惩罚/无slash）
  - [ ] **P2-1(iv) 治理执行 hook（未实现，2026-08-10 review 补录）**：`admit_node` / `allow_model` / `param_change` 批准后目前**无任何执行效果**（`governance.go recompute` 仅置 `Status="ratified"`，payload 只哈希不解析），当前实现实为"审计账本"（trust-through-audit），与父项"节点准入/白名单共治"的执行语义不符。二选一：补执行 hook，或把父项措辞降级为"审计账本"。
...
- [x] P2-2 贡献账本可视化：admin 页面展示"算力从哪来、到哪去"透明度面板
  ...
  - [ ] **P2-2(ii) admin UI 面板（未实现，2026-08-10 review 补录）**：前端尚无页面消费 `/api/admin/ledger/transparency`、`/api/admin/ledger/contribution-quota`、`/api/admin/ledger/export`（grep `.html/.js` 零命中）。父项 [x] 仅代表后端端点完成，交付物"面板"缺失。
...
- [ ] **P5-4 `startRegionSyncLoop` 空转（stubs.go:209-222，2026-08-10 review 补录）**：循环体仅 `sleep` + TODO，无真实跨节点区域对账（`regionManager` 已按节点本地权威，跨节点 reconciliation 未实现）。实现或删除，避免"后台循环在跑但什么都没做"。
