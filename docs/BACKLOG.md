...
- [x] **P1-1 DHT 网络 I/O（单进程多节点验证）**：为 `dht_kademlia.go` 的惰性 K-Bucket 表补上真正的 Kademlia RPC 层（新增 `dht_networking.go`）。实现 `DHTMessage`（PING/PONG、FIND_NODE、FIND_VALUE、STORE）+ `DHTTransport` 可插拔传输 + `DHTNode`（含迭代查找算法）+ `InMemoryDHTNetwork`（单进程内多节点路由，作验证台）。`dht_networking_test.go` 四个用例证明：Ping/Pong、A 仅知 B 却经 B 学到 C 的**多跳发现**、值经 relay 存储并被 D 跨跳取回、缺失键返回未找到。这是 P1-1 的第一个里程碑（先单进程多节点验证）；后续再桥接真实 UDP/QUIC/HTTP 传输。**未改动现有 federation/gossip 代码路径**（加法式、零耦合）。
  - [x] **P1-1(ii) DHT 运输层桥接 + 生产接入（2026-08-16 实现）**：新增 `dht_udp.go` 实现真实 UDP `DHTTransport`（`UDPDHTTransport`：reqID 配对的请求/应答半双工、graceful `Stop` 关 socket）+ `startDHTNode()`/`stopDHTNode()` 生产单例；`init.go::initAllNetwork` 末尾在 `network_enabled` 且 `node.NodeID()!=""` 且 `dht_enabled!=false` 时启动并 bootstrap（`dht_listen_addr` 默认 `:19001`、`dht_seeds` 可配），`server.go::gracefulShutdown` 补 `stopDHTNode()`。`InMemoryDHTNetwork` 仍作测试台保留，与 `federation.go` 既有只读 DHT 路由索引**正交**（不触碰，零耦合）。`dht_udp_test.go` 3 用例（Ping/Pong 回环、多跳 IterativeFindNode、Store/FindValue）全绿。`go build/vet/test ./...` 全绿。注：QUIC 因引入外部依赖被排除，UDP/HTTP 已满足"真实 transport"目标；DHT 当前为生产节点引导，多节点路由随 federation 演进。
...
- [x] P1-2b-2 真实 UDP 打洞 / 直连通道建立（cone NAT 尝试直连：经 relay/gossip 交换 reflexive 地址 + 并发打洞 + 直连通道旁路 relay）
  ...
  - [x] P1-2b-2(iii-2) 接入 relay 真实交换 offer + 直连旁路：`RouteEntry` 增 `ReflexiveUDP`/`NATType`；新增 `/network/__punch` 端点交换 PunchOffer；`relayToRemote` 在 non-symmetric 且有对端 reflexive 时异步 `ExchangePunchWithPeer` + `BeginPunch`，直连建立后跳过冗余 TCP 探测（relay 旁路信号）；`nat_punch_relay_test.go` 2 用例覆盖「经 HTTP 交换 offer → 双向 UDP 打洞建立直连」真实链路。修复打洞协议缺陷：发送须持续到 maxAttempts（NAT 映射需双向持续外出包），不能在收到首帧即停。注：直连通道**承载实际 HTTP 流量**需在 UDP 上自建请求/应答协议（P2 工作，已由 P1-2b-2(iv) 于 2026-08-16 补完 UDP 数据承载），本期达成「打洞真实打通 + 链路可用」目标
  - [x] **P1-2b-2(iv) UDP 直连数据承载协议（2026-08-16 实现，P1-2b-2 补齐项）**：新增 `udp_bearer.go` 实现 `UDPDataBearer`——在已打通的直连 UDP 链路上自建 请求/应答 协议（magic `OMP2` 与打洞 `OMP1`/STUN 区分；envelope 帧 + `B` 分片，1400 字节/片适配 MTU；响应**全量重组后才写** HTTP 客户端，任何失败干净回落 HTTPS 反向代理、无半包）。复用共享单 UDP socket 的 `udpRecvLoop` 多路（单读者无新竞争），复用既有 ed25519 relay 转发签名（`signRelayForward`/`verifyRelayForwardAuth`）做鉴权不引入新信任模型；流式（`stream:true`）请求不承载、回落 HTTPS 保留 token 流式。`nat_traversal.go`：`udpRecvLoop` 将 `OMP2` 帧转交 `udpDataBearer`、`ensureDirectLinkMgr` 在 socket 可用且 `udp_data_bearer_enabled!=false` 时建 `udpDataBearer`；`network_relay.go::relayToRemote` 在 `directLinkMgr.HasDirect` 命中时优先 `RelayOverUDP`，失败回退 HTTPS。`udp_bearer_test.go` 3 用例（回环 relay、50KB 分片重组、流式回落）全绿。`go build/vet/test ./...` 全绿。
...
- [x] P2-1 社区共治设计：节点准入 / 模型白名单多签共治（治理哲学 2026-08-09 已定：默认善意、只防滥发提案、贡献者共治、无惩罚/无slash）
  - [x] **P2-1(iv) 治理执行 hook（2026-08-12 实现，C 方案）**：批准后 `admit_node` 写入"社区策展节点名册"、`allow_model` 写入"社区策展模型白名单"（均加成式 allowlist，`governance.go` 新增 `executeRatified`/`rebuildEffect` + `AdmittedNodes`/`AllowedModels`/`IsCommunityAdmitted`/`IsCommunityCuratedModel`；名册仅用于标注/策展，不剥夺任何现有接入，契合善意默认）；`param_change` 保持审计-only（不解析不应用，避免 2/3 贡献者合谋改安全参数）。名册从账本重建（load 时 `rebuildEffect`），永不偏离防篡改记录。`/api/governance/proposals` 响应新增 `admitted_nodes`/`allowed_models`。`governance_exec_test.go` 5 用例（admit/allow 生效、param 审计-only、重载重建、空 payload no-op）。`go build/vet/test ./...` 全绿
...
- [x] P2-2 贡献账本可视化：admin 页面展示"算力从哪来、到哪去"透明度面板
  ...
  - [x] **P2-2(ii) admin UI 面板（2026-08-11 实现）**：新增 `admin-ledger.js` 透明度面板，消费 `/api/admin/ledger/transparency`（按 peer/model 聚合贡献 token、记录数、trust/claim/penalty/tx 计数、chainValid）、`/api/admin/ledger/contribution-quota`（贡献量↔赚得免费配额 1:1 视图）、`/api/admin/ledger/export?format=csv|json`（研究者导出）；`admin.html` 新增透明度卡片 + 刷新/导出按钮 + footer 版本号动态注入（修掉硬编码 v4.1 漂移）；`embed.go`/`admin_collab.go`/`routes.go` 接入新资源；`ledger_panel_wire_test.go` 3 用例锁接线（卡片片段 + `?v=\d+` 缓存号 + 端点真实字符串 + 公益黑话禁用词护栏）。公益措辞明确"额度按 1:1 等额记账，不是代币：不可交易/不可提现/无手续费/不通胀；用尽回落社区免费池"。`go build/vet/test ./...` 全绿
