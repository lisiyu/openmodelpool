# ModelMux v3.2.0 — 全面回归测试 & 安全性能审查报告
...
**影响**: 默认情况下，任何来源的网页都可以跨域调用 ModelMux API。如果 admin 在浏览器中登录了 ModelMux，恶意网站可以利用 CORS 通配符发起 CSRF 风格的攻击（虽然无法读取响应，但可以发送请求）。
...
ModelMux v3.2.0 是一个功能丰富、架构清晰的 AI 网关。主要改进方向集中在：
